...
def benchmark_fsdp(rank, world_size):
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "12355"
    dist.init_process_group("nccl", rank=rank, world_size=world_size)
    torch.cuda.set_device(rank)
    ...
    model = BasicsTransformerLM(**model_config)
    model.cuda()
    mem_before_fsdp = torch.cuda.memory_allocated()
    if rank == 0:
        print(f"Memory before fsdp: {mem_before_fsdp/1e9:.2f} GB")
    fsdp_model = FSDP(model, compute_dtype=torch.float16)
    optimizer = torch.optim.Adam(fsdp_model.parameters(), lr=0.001)
    mem_after_fsdp = torch.cuda.memory_allocated()
    if rank == 0:
        print(f"Memory after fsdp: {mem_after_fsdp/1e9:.2f} GB")
    torch.cuda.synchronize()
    start_time = timeit.default_timer()
    num_steps = 1
    for _ in range(num_steps):
        output_BLD = fsdp_model(input_BLD)
        loss = output_BLD.sum()
        loss.backward()
    ...
    torch.cuda.synchronize()
    end_time = timeit.default_timer()
    print(f"Average time fsdp forward+backward+update per step: {(end_time - start_time) / num_steps} seconds")
    cleanup()
